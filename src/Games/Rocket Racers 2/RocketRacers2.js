...
var scrollPreventCanvas, trackCanvas, carCanvas, imageCanvas, screenCanvas, backCanvas, coinCanvas;

var trackContext, carContext, imageContext, screenContext, backContext, coinContext; 
...
var raceTrackImage, backImage;
var coinImage, coinEdgeImage;
...
var score = 0;
var bestScore = 0;
var isPingAudioLoaded = 0, isPingAudioLoading = 0, isExplosionAudioLoaded = 0, isExplosionAudioLoading = 0, isIOS = 0, isMobile = 0;

var rotationMatrix = [[1,0,0],[0,1,0,],[0,0,1]];
var phiX = -50*Math.PI/180.;
var phiY = -20*Math.PI/180.;
var nu = 2*Math.PI/180.;
var numOfCoins = 10;
var coinRs = new Array(numOfCoins);
var coinThetas = new Array(numOfCoins);
var isCoinActive = new Array(numOfCoins);
var coinTimer;

function initializeCoins() {
	rotateX(phiX);
	rotateY(phiY);
	for (var i = 0; i < coinThetas.length; i++) {
		coinThetas[i] = i*2*Math.PI/coinThetas.length;
		coinRs[i] = (Math.random() < 0.5 ? innerRad:outerRad);
		isCoinActive[i] = 1;
	}
	coinTimer = setInterval(rotateCoins,10);
}

function rotateCoins() {
	rotateX(-nu*Math.sin(phiX)*Math.sin(phiY));
	rotateY(-nu*Math.cos(phiX));
	rotateZ(-nu*Math.sin(phiX)*Math.cos(phiY));
	renderCoins();
}

function drawCoin(x,y) {
	coinContext.save();
	coinContext.translate(x,y-2);
	coinContext.transform(rotationMatrix[0][0],rotationMatrix[1][0],rotationMatrix[0][1],rotationMatrix[1][1],rotationMatrix[0][2],rotationMatrix[1][2]);
	coinContext.drawImage(coinEdgeImage,-12,-12);
	coinContext.restore();
	coinContext.save();
	coinContext.translate(x,y);
	coinContext.transform(rotationMatrix[0][0],rotationMatrix[1][0],rotationMatrix[0][1],rotationMatrix[1][1],rotationMatrix[0][2],rotationMatrix[1][2]);
	coinContext.drawImage(coinImage,-12,-12);
	coinContext.restore();
}

function renderCoins() {
	coinContext.clearRect(0,0,700,700);
	for (var i = 0; i < coinRs.length; i++) {
		if (isCoinActive[i]) {
			drawCoin(coinRs[i]*Math.cos(coinThetas[i]-Math.PI/2)+350,coinRs[i]*Math.sin(coinThetas[i]-Math.PI/2)+350);
		}
	}
}
...
function createCoinEdge() {
	...
	imageContext.save();
	imageContext.translate(-38,-38);
	imageContext.fillStyle = "darkgoldenrod";
	...
	coinEdgeImage = new Image(24,24);
	coinEdgeImage.onload = createRaceTrack;
	coinEdgeImage.src = imageCanvas.toDataURL("image/png");
}
...
function createGoldCoins() {
	...
	imageContext.save();
	imageContext.translate(-38,-38);
	imageContext.fillStyle = "goldenrod";
	imageContext.beginPath();
	imageContext.moveTo(50,50);
	imageContext.arc(50,50,12,0,2*Math.PI);
	imageContext.closePath();
	...
	imageContext.beginPath();
	imageContext.moveTo(50,50);
	imageContext.arc(50,50,11,0,2*Math.PI);
	imageContext.closePath();
	imageContext.fill();
	imageContext.fillStyle = "darkgoldenrod";
	imageContext.beginPath();
	...
	imageContext.closePath();
	imageContext.fill();
	imageContext.strokeStyle = "rgb(67,51,10)";
	//imageContext.fillStyle = "goldenrod";
	imageContext.lineWidth = 1;
	imageContext.font = "14px impact";
	imageContext.textAlign = "center";
	imageContext.textBaseline = "middle";
	imageContext.strokeText("$",50,50);
	//imageContext.fillText("$",50,50);
	var grad1 = imageContext.createLinearGradient(0,0,100,100); 
	grad1.addColorStop(0.4,"rgba(255,255,255,0)");
	grad1.addColorStop(0.5,"rgba(255,255,255,0.8)");
	grad1.addColorStop(0.6,"rgba(255,255,255,0)");  
	...
	imageContext.fill();
	imageContext.restore();
	coinImage = new Image(24,24);
	coinImage.onload = createCoinEdge;
	coinImage.src = imageCanvas.toDataURL("image/png");
}

function createBackTrack() {
	backContext.clearRect(0,0,1200,700);
	var grad = backContext.createLinearGradient(0,0,1200,700);
	grad.addColorStop(0,"rgb(0,150,0)");
	grad.addColorStop(1,"rgb(0,51,0)");
	backContext.fillStyle = grad;
	backContext.fillRect(0,0,1200,700);
	backImage = new Image(1200,700);
	backImage.onload = createGoldCoins;
	//backImage.onload = createRaceTrack;
...
function centerAllCanvases() {
	...
	centerCanvas(backCanvas);
	centerCanvas(coinCanvas);
}
...
function rescaleCanvases() {
	...
	var offsetX = Math.round(scale*250.);
	trackCanvas.style.left = offsetX + "px";
	...
	carCanvas.style.left = offsetX + "px";
	carCanvas.style.top = "0px";
	carContext.scale(scale,scale);
	screenCanvas.width = Math.round(1200.*scale);
	screenCanvas.height = Math.round(700.*scale);
	screenContext.scale(scale,scale);
	backCanvas.width = Math.round(1200.*scale);
	backCanvas.height = Math.round(700.*scale);
	backCanvas.style.left = "0px";
	backCanvas.style.top = "0px";
	backContext.scale(scale,scale);
	coinCanvas.width = Math.round(700.*scale);
	coinCanvas.height = Math.round(700.*scale);
	coinCanvas.style.left = offsetX + "px";
	coinCanvas.style.top = "0px";
	coinContext.scale(scale,scale);
	imageCanvas.width = 0;
	...
	renderCoins();
	if (gameOver) {
		screenContext.drawImage(replayImage,550,300);
	}
}
...
function drawBestScore() {
	...
	screenContext.font = "30px helvetica";
	screenContext.textAlign = "left";
	screenContext.textBaseline = "middle";
	screenContext.strokeText("Best Score: " + bestScore + "",50,50);
	screenContext.fillText("Best Score: " + bestScore + "",50,50);
}

function drawScore() {
	screenContext.clearRect(0,0,1200,700);
	screenContext.strokeStyle = "black";
	screenContext.fillStyle = "white";
	...
	screenContext.textBaseline = "middle";
	screenContext.strokeText(score + "",1120,50);
	screenContext.fillText(score + "",1120,50);
	screenContext.drawImage(coinImage,0,0,24,24,1150-18,50-18,36,36);
	drawBestScore();
}
...
function moveCars() {
	...
	for (var i = 0; i < numOfCoins; i++) {
		if (R1 === coinRs[i] && (theta1+phi) >= coinThetas[i] && (theta1-phi) <= coinThetas[i] && isCoinActive[i]) {
			score++
			drawScore();
			playPingSound();
			isCoinActive[i] = 0;
			if (score % numOfCoins === 0) {
...
function createReplayButton() {
	...
	imageContext.arc(imageCanvas.width/2,imageCanvas.height/2,42,0,2*Math.PI);
	imageContext.closePath();
	imageContext.fill();
	...
	imageContext.beginPath();
	imageContext.moveTo(imageCanvas.width/2,imageCanvas.height/2);
	imageContext.arc(imageCanvas.width/2,imageCanvas.height/2,38,0,2*Math.PI);
	imageContext.closePath();
	imageContext.fill();
...
function createRaceTrack() {
	...
	trackContext.arc(trackCanvas.width/2,trackCanvas.height/2,r2,0,2*Math.PI);
	trackContext.closePath();
	trackContext.fill();
	trackContext.fillStyle = "rgb(51,51,51)";
	trackContext.beginPath();
	trackContext.moveTo(trackCanvas.width/2,trackCanvas.height/2);
	trackContext.arc(trackCanvas.width/2,trackCanvas.height/2,r1,0,2*Math.PI);
	trackContext.closePath();
	trackContext.fill();
	trackContext.fillStyle = "rgb(0,51,0)";
	trackContext.beginPath();
	trackContext.moveTo(trackCanvas.width/2,trackCanvas.height/2);
	trackContext.arc(trackCanvas.width/2,trackCanvas.height/2,r4,0,2*Math.PI);
	trackContext.closePath();
	trackContext.fill();
	var grad = trackContext.createRadialGradient(trackCanvas.width/2,trackCanvas.height/2,r6,trackCanvas.width/2,trackCanvas.height/2,r5);
	grad.addColorStop(0,"rgb(0,51,0)");
	grad.addColorStop(0.8,"rgb(0,102,0)");
	trackContext.fillStyle = grad;
	trackContext.beginPath();
	trackContext.moveTo(trackCanvas.width/2,trackCanvas.height/2);
	trackContext.arc(trackCanvas.width/2,trackCanvas.height/2,r5,0,2*Math.PI);
	trackContext.closePath();
	trackContext.fill();
	createTrackMarks();
	createStartRace();
...
function createTrackMarks() {
	...
	for(var i = 0; i < 2*Math.PI; i += Math.PI/20.) {
		trackContext.moveTo(r1*Math.cos(i)+trackCanvas.width/2,r1*Math.sin(i)+trackCanvas.height/2);
		trackContext.arc(trackCanvas.width/2,trackCanvas.height/2,r1,i,i+Math.PI/40.);		
	}
	trackContext.stroke();
}
...
function createLaneMarks() {
	...
	for(var i = 0; i < 2*Math.PI; i += Math.PI/10.) {
		trackContext.moveTo(r2*Math.cos(i)+trackCanvas.width/2,r2*Math.sin(i)+trackCanvas.height/2);
		trackContext.arc(trackCanvas.width/2,trackCanvas.height/2,r2,i,i+Math.PI/20.);
	}
	trackContext.stroke();
	trackContext.strokeStyle = "red";
	trackContext.lineWidth = 5;
	trackContext.beginPath();
	for(var i = Math.PI/20; i < 2*Math.PI + Math.PI/20; i += Math.PI/10.) {
		trackContext.moveTo(r2*Math.cos(i)+trackCanvas.width/2,r2*Math.sin(i)+trackCanvas.height/2);
		trackContext.arc(trackCanvas.width/2,trackCanvas.height/2,r2,i,i+Math.PI/20.);
	}
	trackContext.stroke();
	...
	trackContext.beginPath();
	for(var i = 0; i < 2*Math.PI; i += Math.PI/10.) {
		trackContext.moveTo(r4*Math.cos(i)+trackCanvas.width/2,r4*Math.sin(i)+trackCanvas.height/2);
		trackContext.arc(trackCanvas.width/2,trackCanvas.height/2,r4,i,i+Math.PI/20.);
	}
	trackContext.stroke();
	trackContext.strokeStyle = "red";
	trackContext.lineWidth = 3;
...
function createStartRace() {
	...
	trackContext.fillRect(trackCanvas.width/2 - 20,trackCanvas.height/2 - r2,40,140);
	trackContext.strokeStyle = "white";
	trackContext.lineWidth = 3;
	trackContext.strokeRect(trackCanvas.width/2 - 20, trackCanvas.height/2 - r2+1, 40, 139);
	trackContext.fillStyle = "white";
	for (var y = 0; y < 140; y += 40) {
		trackContext.fillRect(trackCanvas.width/2 - 20,trackCanvas.height/2 - r2 + y,20,20);
	}
	for (var y = 20; y < 140; y += 40) {
		trackContext.fillRect(trackCanvas.width/2 ,trackCanvas.height/2 - r2 + y,20,20);
	}
}
...
function createCar2() {
	...
	carImage2.onload = createBackTrack;
}
...
function drawRing0() {
	...
	screenContext.fillRect(0,0,1200,700);
	imageCanvas.width = imageCanvas.width;
	imageContext.fillStyle = "orange";
	imageContext.beginPath();
	imageContext.moveTo(50,50);
...
export class RocketRacers2 extends React.Component {
    ...
    render() {
        return (
            <div>
            <canvas id ="scrollPreventCanvas" width="100" height="100" style={{position: "absolute", top: "100", left: "100", zIndex: 0, border: "0px solid black"}} />
            <canvas id ="imageCanvas" width="100" height="100" style={{position: "absolute", top: "100", left: "100", zIndex: 1, border: "0px solid black"}} />
            <canvas id ="backCanvas" width="1200" height="700" style={{position: "absolute", top: "0", left: "0", zIndex: 2, border: "0px solid black"}} />
            <canvas id ="trackCanvas" width="700" height="700" style={{position: "absolute", top: "0", left: "250", zIndex: 3, border: "0px solid black"}} />
            <canvas id ="coinCanvas" width="700" height="700" style={{position: "absolute", top: "0", left: "250", zIndex: 4, border: "0px solid black"}} />
            <canvas id ="carCanvas" width="110" height="100" style={{position: "absolute", top: "0", left: "0", zIndex: 5, border: "0px solid black"}} />
            <canvas id ="screenCanvas" width="1200" height="700" style={{position: "absolute", top: "0", left: "0", zIndex: 6, border: "0px solid black"}} />
            </div>
        );
    }
}
