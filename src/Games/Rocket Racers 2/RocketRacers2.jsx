...
function renderCoins() {
  ...
  for (var i = 0; i < coinRs.length; i++) {
    if (isCoinActive[i]) {
      drawCoin(
        coinRs[i] * Math.cos(coinThetas[i] - Math.PI / 2) + 350,
        coinRs[i] * Math.sin(coinThetas[i] - Math.PI / 2) + 350
      );
    }
  }
}
...
function createRaceTrack() {
  ...
  trackContext.closePath();
  trackContext.fill();
  var grad = trackContext.createRadialGradient(
    trackCanvas.width / 2,
    trackCanvas.height / 2,
    r6,
    trackCanvas.width / 2,
