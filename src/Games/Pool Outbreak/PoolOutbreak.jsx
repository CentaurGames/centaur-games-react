...
function drawBall() {
  ...
  for (var i = 0; i < xs.length; i++) {
    ...
    if (explosiveIndex[i] != -1) {
      explodingContext.drawImage(
        explosiveImages[explosiveIndex[i]],
        xs[i] - r - 40,
        ys[i] - r - 40
      );
    }
...
function spreadContagion(j) {
  ballTimer[j] = setInterval(function () {
    explosionCountDown(j);
  }, 1000);
