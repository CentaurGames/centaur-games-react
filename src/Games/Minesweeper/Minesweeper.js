...
function centerAllCanvases() {
  ...
  var offsetY = Math.floor(
    (window.innerHeight - scrollPreventCanvas.height) / 2
  );
  offsetY -= rect.top;
...
function drawSmiley2() {
  ...
  imageContext.arc(
    Math.floor(smileyWidth / 2),
    Math.floor(smileyHeight / 2),
    Math.floor(smileyWidth / 2.5),
    0,
    2 * Math.PI
  );
...
function drawTile(i, j) {
  ...
  tileContext.closePath();
  tileContext.fill();
  var gradient = tileContext.createLinearGradient(
    x + Math.round(pixelWidth / 10),
    y + Math.round(pixelHeight / 10),
    x + pixelWidth - Math.round(pixelWidth / 10),
    y + pixelHeight - Math.round(pixelHeight / 10)
  );
  ...
  tileContext.fillRect(
    x + Math.round(pixelWidth / 10),
    y + Math.round(pixelHeight / 10),
    Math.round((4 * pixelWidth) / 5),
    Math.round((4 * pixelHeight) / 5)
  );
  return;
}
...
function drawNumberTile(i, j, number) {
  ...
  if (number == -1) {
    ...
    numberContext.lineTo(
      x + Math.round((3 * pixelWidth) / 8),
      y + Math.round((3 * pixelHeight) / 8)
    );
...
function drawGreenFlag(i, j) {
  ...
  flagContext.fillRect(
    x - Math.round(pixelWidth / 20),
    y + Math.round(pixelHeight / 8),
    Math.round(pixelWidth / 10),
    Math.round((3 * pixelHeight) / 4)
  );
  flagContext.fillStyle = "green";
  flagContext.beginPath();
  flagContext.moveTo(x, y + Math.round(pixelHeight / 8));
  flagContext.lineTo(
    x - Math.round(pixelWidth / 3),
    y + Math.round((2.5 * pixelHeight) / 8)
  );
  ...
  flagContext.fillRect(
    x - Math.round(pixelWidth / 3),
    y + Math.round((3 * pixelHeight) / 4),
    Math.round((2 * pixelWidth) / 3),
    Math.round(pixelHeight / 8)
  );
  return;
}
...
function shiftClick(x, y) {
  ...
  flagContext.fillStyle = "black";
  flagContext.fillRect(
    x1 - Math.round(pixelWidth / 20),
    y1 + Math.round(pixelHeight / 8),
    Math.round(pixelWidth / 10),
    Math.round((3 * pixelHeight) / 4)
  );
  flagContext.fillStyle = "red";
  ...
  flagContext.lineTo(
    x1 - Math.round(pixelWidth / 3),
    y1 + Math.round((2.5 * pixelHeight) / 8)
  );
...
function clickStuff(e) {
  ...
  if (
    e.clientX >= smileyRect.left &&
    e.clientX <= smileyRect.right &&
    e.clientY >= smileyRect.top &&
    e.clientY <= smileyRect.bottom
  ) {
    gameOver = 0;
    if (isMyTimerRunning) {
      clearInterval(myTimer);
