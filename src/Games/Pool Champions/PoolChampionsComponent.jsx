...
function collideBalls(i, j) {
  ...
  Vys[j] = newVy1;
  var V = Math.sqrt(
    (Vxs[i] - Vxs[j]) * (Vxs[i] - Vxs[j]) +
      (Vys[i] - Vys[j]) * (Vys[i] - Vys[j])
  );
  if (V > 4) {
    audioVolume.gain.setValueAtTime(1, audioContext.currentTime);
  } else {
    audioVolume.gain.setValueAtTime(
      Math.pow(V / 4, 5.6),
...
function correctOverlaps() {
  ...
  for (var i = 0; i < xs.length; i++) {
    for (var j = 0; j < ys.length; j++) {
      if (i == j || !isBallOnTable[i] || !isBallOnTable[j]) {
        continue;
      }
      dx = xs[i] - xs[j];
      dy = ys[i] - ys[j];
