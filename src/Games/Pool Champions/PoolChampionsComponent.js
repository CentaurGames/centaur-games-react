...
var scrollPreventCanvas, myCanvas, ballCanvas, tableCanvas, cueCanvas, cursorCanvas, imageCanvas, floorCanvas, scoreCanvas, shinyCanvas, shadowCanvas;

var context, tableContext, cueContext, cursorContext, imageContext, floorContext, scoreContext, shinyContext, shadowContext;
...
var colors = ["white","goldenrod","blue","red","purple","rgb(255,100,0)","green","maroon","black","goldenrod","blue","red","purple","rgb(255,100,0)","green","maroon"];
var rotTimer;
var gradientImage = new Image(2*r,2*r);
var isPlayerSolids = 0;
var shadowImage = new Image(2*r,2*r);

var scene, camera, renderer, spheres = new Array(16), sphereCameras = new Array(16);
...
function determineTeams(ballIndex) {
	...
	isPlayerSolids = (ballIndex < 8 ? isOpponentPlaying : !isOpponentPlaying);
}

function updateScore(ballIndex) {
	determineTeams(ballIndex);
	if (isPlayerSolids) {
		if (ballIndex < 8) {
			playerScore++;
...
function initialize3d() {
	renderer = new THREE.WebGLRenderer({canvas: ballCanvas, antialias: true, alpha: true});
	renderer.setClearColor(0x000000,0);
	renderer.setSize(ballCanvas.width,ballCanvas.height);
	camera = new THREE.OrthographicCamera(-ballCanvas.width/2,ballCanvas.width/2,-ballCanvas.height/2,ballCanvas.height/2,0.1,10000);
	camera.position.set(ballCanvas.width/2,ballCanvas.height/2,400);
	scene = new THREE.Scene();
	scene.add(camera);
	var light = new THREE.AmbientLight(0xffffff,0.3);
	scene.add(light);
	light = new THREE.DirectionalLight(0xffffff,1.0);
	light.position.set(ballCanvas.width/2,ballCanvas.height/2,100);
	light.target.position.set(ballCanvas.width/2,ballCanvas.height/2,0);
	scene.add(light);
	light = new THREE.SpotLight(0xffffff,1.0);
	light.position.set(0,0,10);
	light.angle = 15*Math.PI/180;
	light.target.position.set(ballCanvas.width/2,ballCanvas.height/2,0);
	scene.add(light);
	light = new THREE.SpotLight(0xffffff,1.0);
	light.position.set(ballCanvas.width,0,10);
	light.angle = 15*Math.PI/180;
	light.target.position.set(ballCanvas.width/2,ballCanvas.height/2,0);
	scene.add(light);
	light = new THREE.SpotLight(0xffffff,1.0);
	light.position.set(0,ballCanvas.height,10);
	light.angle = 15*Math.PI/180;
	light.target.position.set(ballCanvas.width/2,ballCanvas.height/2,0);
	scene.add(light);
	light = new THREE.SpotLight(0xffffff,1.0);
	light.position.set(ballCanvas.width,ballCanvas.height,10);
	light.angle = 15*Math.PI/180;
	light.target.position.set(ballCanvas.width/2,ballCanvas.height/2,0);
	scene.add(light);
	var loader = new THREE.TextureLoader();
...
function drawGradientImage() {
	...
	imageContext.scale(r/25,r/25);
	var grad = imageContext.createRadialGradient(10,19,0,10,19,43);
	...
	imageContext.moveTo(25,25);
	imageContext.arc(25,25,25,0,2*Math.PI);
	imageContext.closePath();
	imageContext.fill();
	grad = imageContext.createRadialGradient(25,25,0,25,25,25);
	grad.addColorStop(0,"rgba(255,255,255,0)");
	grad.addColorStop(0.88,"rgba(255,255,255,1)");
	imageContext.fillStyle = grad;
	imageContext.beginPath();
	imageContext.moveTo(12,12);
	imageContext.arc(25,25,18,5*Math.PI/4,Math.PI,true);
	imageContext.lineTo(3,25);
	imageContext.arc(25,25,22,Math.PI,5*Math.PI/4);
	imageContext.lineTo(12,12);
	imageContext.closePath();
	imageContext.fill();
	r--;
...
function drawShadowImage() {
	imageCanvas.width = 2*r;
	imageCanvas.height = 2*r;
	imageContext.fillStyle = "rgba(0,0,0,0.3)";
	imageContext.beginPath();
	imageContext.moveTo(r,r);
	imageContext.arc(r,r,r,0,2*Math.PI);
	imageContext.closePath();
	imageContext.fill();
	shadowImage.src = imageCanvas.toDataURL("image/png");
	shadowImage.onload = drawGradientImage;
}
...
function updateRotation() {
	...
	for (var i = 0; i < 16; i++) {
		wx = -Vys[i] / r;
		wy = Vxs[i] / r;
		//spheres[i].rotation.x += wx;
		//spheres[i].rotation.y += wy;
		rotateX(-wx,i);
		rotateY(-wy,i);
		m.set(
			rotMatrices[i][0][0], rotMatrices[i][0][1], rotMatrices[i][0][2], 0,
			rotMatrices[i][1][0], rotMatrices[i][1][1], rotMatrices[i][1][2], 0,
			rotMatrices[i][2][0], rotMatrices[i][2][1], rotMatrices[i][2][2], 0,
			0, 0, 0, 1 
		);
		spheres[i].rotation.setFromRotationMatrix(m);
	}
}
...
function carpetPattern() {
	...
	imageContext.strokeStyle = "darkgoldenrod"    
...
function runAI() {
	...
	var holeX = findNearestHoleX(index);
	var holeY = findNearestHoleY(index);
	var dx = holeX - xs[index];
	var dy = holeY - ys[index];
	var theta = Math.atan2(dy,dx);
	var newX = xs[index] - 2*r*Math.cos(theta);
	var newY = ys[index] - 2*r*Math.sin(theta);
	dx = xs[0] - newX;
	dy = ys[0] - newY;
...
function rescaleCanvases() {
	...
	ballCanvas.style.left = "0px";
	ballCanvas.style.top = "0px";
	shinyCanvas.style.left = "0px";
	shinyCanvas.style.top = "0px";
	shadowCanvas.style.left = "0px";
	shadowCanvas.style.top = "0px";
	...
	ballCanvas.width = Math.round(800.*scale);
	ballCanvas.height = Math.round(520.*scale);
	//offsetY = Math.round(scale*0.);
	//ballCanvas.style.top = offsetY + "px";
	//offsetX = Math.round(scale*0.);
	//ballCanvas.style.left = offsetX + "px";
	renderer.setSize(ballCanvas.width,ballCanvas.height);
	shinyCanvas.width = Math.round(800.*scale);
	shinyCanvas.height = Math.round(520.*scale);
	//offsetY = Math.round(scale*0.);
	//shinyCanvas.style.top = offsetY + "px";
	//offsetX = Math.round(scale*0.);
	//shinyCanvas.style.left = offsetX + "px";
	shinyContext.scale(scale,scale);
	shadowCanvas.width = Math.round(800.*scale);
	shadowCanvas.height = Math.round(520.*scale);
	//offsetY = Math.round(scale*0.);
	//shadowCanvas.style.top = offsetY + "px";
	//offsetX = Math.round(scale*0.);
	//shadowCanvas.style.left = offsetX + "px";
	shadowContext.scale(scale,scale);
	cueCanvas.width = 0;//Math.round(370.*scale);
	cueCanvas.height = 0;//Math.round(15.*scale);
	...
	imageCanvas.width = 0;//Math.round(20.*scale);
	imageCanvas.height = 0;//Math.round(20.*scale);
	//offsetY = Math.round(scale*50.);
...
function drawPlayerScore(i) {
	if ((isPlayerSolids && i < 8) || (!isPlayerSolids && i > 8)) {
		xs[i] = -30 + playerScore*2*r*1.5;
		ys[i] = -65;
	} else {
		xs[i] = 670 - AIScore*2*r*1.5;
		ys[i] = -65;
	}
	//spheres[i].rotation.y = -Math.PI/2;
	//spheres[i].rotation.x = 0;
	//spheres[i].rotation.z = Math.PI;
	rotMatrices[i] = [[1,0,0],[0,1,0],[0,0,1]];
	rotateY(Math.PI/2,i);
	rotateZ(Math.PI,i);
	drawPlayerInfo();
}
...
function drawPlayerInfo() {
	...
	scoreContext.strokeText("Player",380,25);
	scoreContext.fillText("Player",380,25);
	scoreContext.textAlign = "left";
	scoreContext.strokeText("Computer",420,25);
	scoreContext.fillText("Computer",420,25);
}

function paintFloorCanvas() {
	var grad = floorContext.createLinearGradient(0,0,800,520);
	grad.addColorStop(0,"rgb(0,0,51)");
	grad.addColorStop(1,"rgb(0,0,102)");
...
function createBallImage(i) {
	...
	imageCanvas.height = 64;
	if (i > 15) {
		initialize3d();
		return;
	}
	imageCanvas.width = imageCanvas.width;
	imageContext.fillStyle = colors[i];
	...
	imageContext.arc(imageCanvas.width/2,imageCanvas.height/2,r,0,2*Math.PI);
	imageContext.closePath();
	imageContext.fill();
	if (i > 8) {
		imageContext.fillStyle = "white";
		imageContext.fillRect(0,0,imageCanvas.width,imageCanvas.height/4);
		imageContext.fillRect(0,3*imageCanvas.height/4,imageCanvas.width,imageCanvas.height/4);
	}
	if (i != 0) {
		...
		imageContext.font = "15px helvetica";
		imageContext.fillText("" + i,imageCanvas.width/2,imageCanvas.height/2);
	}
...
function findNearestHoleX(i) {
	return Math.round(xs[i]/320.)*320.;
}

function findNearestHoleY(i) {
	return Math.round(ys[i]/360.)*360.;
}
...
function nearestHoleToBall(i) {
	...
	if (playerScore != 0 || AIScore != 0 ) {
		if (isPlayerSolids && i < 8) {
			return 100000;
		} else if (!isPlayerSolids && i > 8) {
			return 100000;
		} else if (i === 8 && AIScore < 7) {
			return 100000;
		}
	} else if (i === 8) {
		return 100000;
...
function drawBall() {
	...
	shinyContext.clearRect(0,0,800,520);
	shadowContext.clearRect(0,0,800,520);
	for (var i = 0; i < 16; i++) {
		spheres[i].position.x = xs[i]+80;
		spheres[i].position.y = ys[i]+80;
	}
	renderer.render(scene,camera);
	
	for (var i = 0; i < 16; i++) {
		if (isBallOnTable[i] || (i != 0 && i != 8)) {
			//shinyContext.drawImage(gradientImage,xs[i]-r-1+80,ys[i]-r-1+80);
			shadowContext.drawImage(shadowImage,xs[i]-r-1.5+80,ys[i]-r+80-1.5);
		}
	}
...
function endGame(didPlayerWin) {     // You Win Text function
	gameOver = 1;
...
export class PoolChampions extends React.Component {
    componentDidMount() {
        ...
        shinyCanvas = document.getElementById("shinyCanvas");
        shinyContext = shinyCanvas.getContext("2d");
    ...
    render() {
        return (
            ...
            <canvas id ="myCanvas" width="640" height="360" style={{position: "absolute", top: "80", left: "80", zIndex: 3, border: "0px solid black"}} />
            <canvas id ="ballCanvas" width="800" height="520" style={{position: "absolute", top: "0", left: "0", zIndex: 5, border: "0px solid black"}} />
            <canvas id ="shadowCanvas" width="800" height="520" style={{position: "absolute", top: "0", left: "0", zIndex: 4, border: "0px solid black"}} />
            <canvas id ="shinyCanvas" width="800" height="520" style={{position: "absolute", top: "0", left: "0", zIndex: 6, border: "0px solid black"}} />
            <canvas id ="tableCanvas" width="700" height="420" style={{position: "absolute", top: "50", left: "50", zIndex: 2, border: "0px solid black"}} />
