...
import {treeDataURL, mushroomDataURL, eggDataURL, replayDataURL, scoreDataURL} from "./ChickenWingsImageSources";
...
var canvasWidth = 640, canvasHeight = 1136;
var mushroomWidth = 40, mushroomHeight = 20; //Width and height of the mushroom image. These are placeholder values.
var mushroomRadius = 75;
var eggWidth = 20, eggHeight = 20; //Width and height of the egg image. These are placeholder values.

var numberOfMushroomsPerScreen = 10;

var eggX;
var mushroomXs = new Array(numberOfMushroomsPerScreen), mushroomYs = new Array(numberOfMushroomsPerScreen);
var mushroomScale = new Array(numberOfMushroomsPerScreen);

var scrollSpeed = 15, treeScrollY = 0, scrollInterval, isScrollIntervalRunning = 0;
...
var backgroundAudioContext, backgroundBuffer,  backgroundAudioVolume, isAudioLoaded = 0, isAudioLoading = 0, isIOS, isMobile;
...
function renderScore() {
	...
	var scale = 1.3; 
	replayContext.drawImage(scoreImage,(canvasWidth-scale*scoreImage.width)/2,(canvasHeight-scale*scoreImage.height)/2,scale*scoreImage.width,scale*scoreImage.height);
	...
	replayContext.strokeText("CURRENT SCORE: " + score,canvasWidth/2,canvasHeight/2-20);
	replayContext.fillText("CURRENT SCORE: " + score,canvasWidth/2,canvasHeight/2-20);
	replayContext.strokeText("BEST SCORE: " + bestScore,canvasWidth/2,canvasHeight/2+20);
	replayContext.fillText("BEST SCORE: " + bestScore,canvasWidth/2,canvasHeight/2+20);
	replayContext.font = "20px helvetica";
	replayContext.fillText("CLICK TO PLAY",canvasWidth/2,canvasHeight/2+60);
}
...
function renderTree() {
	...
	treeContext.drawImage(treeImage,0,treeScrollY,canvasWidth,canvasHeight,0,0,canvasWidth,canvasHeight);
}
...
function renderMushroom(i) {
	...
	//Outputs:
		//This function is void.
	mushroomContext.drawImage(mushroomImage,0,0,mushroomWidth,mushroomHeight,mushroomXs[i]-Math.round(mushroomWidth*mushroomScale[i]/2),mushroomYs[i]-Math.round(mushroomHeight*mushroomScale[i]/2),Math.round(mushroomWidth*mushroomScale[i]),Math.round(mushroomHeight*mushroomScale[i]));
}
...
function renderEgg() {
	...
	eggContext.drawImage(eggImage,0,0,eggImage.width,eggImage.height,eggX-width/2,canvasHeight/2-height/2,width,height);
}
...
function renderReplayButton() {
	...
	replayContext.drawImage(replayImage,(canvasWidth-replayImage.width)/2,(canvasHeight-replayImage.height)/2);
}
...
function mouseMoveEvent(evt) {
	...
	eggX = Math.round(((evt.clientX || evt.targetTouches[0].clientX) - rect.left)*scale);
	if (eggX < 0) eggX = 0;
...
function generateMushrooms() {
	...
	for (var i = 0; i < numberOfMushroomsPerScreen; i++) {
		mushroomXs[i] = Math.floor(Math.random()*canvasWidth);
		mushroomYs[i] = i*canvasHeight/numberOfMushroomsPerScreen+canvasHeight;
		mushroomScale[i] = Math.random()*0.75+0.25;
	}
}
...
function initializeGame() {
	...
	//Outputs:
		//This function is void.
	if (/iPhone|iPad|iPod|Android|webOS|Blackberry|IEMobile|Opera Mini/i.test(navigator.userAgent)) isMobile = 1;
...
export class ChickenWings extends React.Component {
    ...
    render() {
        return (
            <div>
                <canvas id="scrollPreventCanvas" width="640" height="1136" style={{position: "absolute", top: "0", left: "0", zIndex: 0, border: "0px solid black"}} />
                <canvas id="treeCanvas" width="640" height="1136" style={{position: "absolute", top: "0", left: "0", zIndex: 1, border: "0px solid black"}} />
                <canvas id="mushroomCanvas" width="640" height="1136" style={{position: "absolute", top: "0", left: "0", zIndex: 2, border: "0px solid black"}} />
                <canvas id="eggCanvas" width="640" height="1136" style={{position: "absolute", top: "0", left: "0", zIndex: 3, border: "0px solid black"}} />
                <canvas id="replayCanvas" width="640" height="1136" style={{position: "absolute", top: "0", left: "0", zIndex: 4, border: "0px solid black"}} />
            </div>
        );
    }
}
