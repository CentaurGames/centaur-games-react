...
import {cavernImageURL, lavaImageURL, tileImageURLs, cartImageURL, coinEdgeURL, coinImageURL} from "./VolcanoFrenzyImageSources";
...
var scrollPreventCanvas, lavaCanvas, cavernCanvas, fallingCanvas, tileCanvas, goldCanvas, cartCanvas, placardCanvas;

var lavaContext, cavernContext, fallingContext, tileContext, goldContext, cartContext, placardContext, playerContext;

var canvasWidth = 1136, canvasHeight = 640;
var gridWidth = 1116, gridHeight = 620;
var tileWidth = 31, tileHeight = 31;
var numberOfTilesX = gridWidth/tileWidth, numberOfTilesY = gridHeight/tileHeight;
var numberOfCartTransitions = 8, transitionTime = 60/numberOfCartTransitions;
var grid, levelGrid, levelPositionX, levelPositionY, levelDirection;

var cavernImage, lavaImage, tileImages = new Array(tileImageURLs.length), cartImage, coinImage, coinEdge;

var rotationMatrix, thetaX, thetaY, thetaZ, phiX, phiY, omega = 2*Math.PI/180;
var scrollY = 0;
var goldX, goldY;
var cartX, cartY, cartDirection, cartScale = 1;
var gameOver = 0, stopRendering = 0, isPlacardOn = 0;;
var score = 0;
var coinInterval, isCoinIntervalRunning = 0;

var coinsInCart, fallingDebrisX, fallingDebrisY, fallingDebrisScale, fallingDebrisIndex, fallingDebrisMatrix, fallingCartInterval, isFallingCartIntervalRunning = 0, fallingCartX, fallingCartY, fallingFactor = 0.98;

var cartIsRolling = 0, lastKeyEvent, canIGetAKeyInEdgewise = 0;
var eventListenersAreSet = 0;
var deviceIsMobile;
var currentLevel = 0, numberOfRectangles;
var swipeStartX, swipeStartY, swipeDirection;
var ignoreNextMove, swipeListen = 0;
...
function setLavaInterval() {
	...
	var lavaInterval = setInterval(function() {
		...
		lavaContext.drawImage(lavaImage,0,scrollY,canvasWidth,canvasHeight,0,0,canvasWidth,canvasHeight);
	},10);
}
...
function drawFallingDebris() {
	...
	for (var i = 0; i < fallingDebrisX.length; i++) {
		...
		fallingContext.save();
		fallingContext.translate(fallingDebrisX[i]+width*scale/2,fallingDebrisY[i]+height*scale/2);
		fallingContext.transform(matrix[0][0],matrix[1][0],matrix[0][1],matrix[1][1],matrix[0][2],matrix[1][2]);
		fallingContext.translate(-fallingDebrisX[i]-width*scale/2,-fallingDebrisY[i]-height*scale/2);
		fallingContext.drawImage(tileImages[fallingDebrisIndex[i]-1],0,0,width,height,fallingDebrisX[i],fallingDebrisY[i],width*scale,height*scale);
...
function drawRotatedCoin(index,i,j) {
	...
	goldContext.transform(rotationMatrix[0][0],rotationMatrix[1][0],rotationMatrix[0][1],rotationMatrix[1][1],rotationMatrix[0][2],rotationMatrix[1][2]);
...
function drawCoins() {
	...
	for (var i = 0; i < goldX.length; i++) {
		drawRotatedCoin(grid[goldX[i]][goldY[i]],left+goldX[i]*tileWidth,top+goldY[i]*tileHeight);
	}
}
...
function drawCoinsInCart() {
	...
	var x1 = (isFallingCartIntervalRunning ? fallingCartX : (left + cartX*tileWidth + tileWidth/2 - cartWidth/4));
	var y1 = (isFallingCartIntervalRunning ? fallingCartY : (top + cartY*tileHeight + tileHeight/2 - cartHeight/4));
	...
	for (var i = 0; i < coinsInCart.length; i++) {
		...
		playerContext.drawImage(coinImage,0,0,coinImage.width,coinImage.height,x1+coinX*coinWidth,y1+coinY*coinHeight-coinZ*coinDepth,coinWidth,coinHeight);
	}
}
...
function drawPlacard() {
	...
	placardContext.fillRect((canvasWidth-width)/2,(canvasHeight-height)/2,width,height);
	...
	placardContext.fillText("LEVEL " + (currentLevel+1),canvasWidth/2,canvasHeight/2);
}
...
function drawScore() {
	...
	placardContext.fillText("Level: " + (currentLevel+1),canvasWidth/4,height/2);
...
function render() {
	...
	for (var i = 0; i < grid.length; i++) {
		for (var j = 0; j < grid[i].length; j++) {
			if (grid[i][j]) tileContext.drawImage(tileImages[grid[i][j]-1],left+i*tileWidth,top+j*tileHeight);
		}
	}
	drawCoins();
	if (stopRendering) return;
	if (!isFallingCartIntervalRunning) {
		playerContext.save();
		playerContext.translate(left+cartX*tileWidth+tileWidth/2,top+cartY*tileHeight+tileHeight/2);
		playerContext.rotate(cartDirection*Math.PI/2);
		playerContext.translate(-left-cartX*tileWidth-tileWidth/2,-top-cartY*tileHeight-tileHeight/2);
		playerContext.drawImage(cartImage,0,0,cartImage.width,cartImage.height,left+cartX*tileWidth+tileWidth/2-cartImage.width*cartScale/2,top+cartY*tileHeight+tileHeight/2-cartImage.height*cartScale/2,cartImage.width*cartScale,cartImage.height*cartScale);
		drawCoinsInCart();
		...
		playerContext.rotate(cartDirection*Math.PI/2);
		playerContext.translate(-fallingCartX,-fallingCartY);
		playerContext.drawImage(cartImage,0,0,cartImage.width,cartImage.height,fallingCartX-cartScale*cartImage.width/2,fallingCartY-cartScale*cartImage.height/2,cartScale*cartImage.width,cartScale*cartImage.height);
		drawCoinsInCart();
		playerContext.restore();
	}
}
...
function moveCart(newDirection,newX,newY) {
	...
	if (newX < 0 || newY < 0 || newX >= grid.length || newY >= grid[0].length || !grid[newX][newY]) {
		gameOverSequence();
		return;
	}
	for (var i = 0; i < goldX.length; i++) {
	...
	if (grid[cartX][cartY] === 7) {
		...
		fallingDebrisMatrix.push([[1,0,0],[0,1,0],[0,0,1]]);
	...
	setTimeout(function() {
		if (cartDirection === 0 && newDirection === 3) {
			...
			transitionBetweenTiles(cartDirection,cartX,cartY,newDirection,newX,newY,0);
		}
	},transitionTime);
}
...
function determineNextMove() {
	if (gameOver || isFallingCartIntervalRunning) return;
	var nextDirection = cartDirection, nextX = cartX, nextY = cartY;
...
function transitionBetweenTiles(oldDirection,oldX,oldY,newDirection,newX,newY,index) {
	if (gameOver || isFallingCartIntervalRunning) return;
	...
	cartDirection = index*(newDirection-oldDirection)/numberOfCartTransitions + oldDirection;
	cartX = index*(newX-oldX)/numberOfCartTransitions + oldX;
	cartY = index*(newY-oldY)/numberOfCartTransitions + oldY;
	render();
	setTimeout(function() {
		transitionBetweenTiles(oldDirection,oldX,oldY,newDirection,newX,newY,index+1);
	},transitionTime);
}
...
function arrowKeyEvent(evt) {
	...
	var newX = cartX, newY = cartY, newDirection = cartDirection;
	switch (evt.keyCode) {
		case 37: //left
			newDirection = 3;
			newX--;
...
function Rectangle(x,y,w,h) {
	...
	this.checkForIntersect = function(rect) {
		if ((this.left === rect.left) || ((this.left+this.width) === (rect.left+rect.width)) || (this.top === rect.top) || ((this.top+this.height) === (rect.top+rect.height))) return 1;
		if ((this.left === (rect.left+rect.width)) || ((this.left+this.width) === rect.left) || (this.top === (rect.top+rect.height)) || ((this.top+this.height) === rect.top)) return 1;
		return 0;
	};
	this.checkArrayForIntersect = function(rects) {
		if (this.top < 0 || this.left < 0 || (this.top+this.height) >= grid[0].length || (this.left+this.width) >= grid.length) return 1;
...
function centerGrid() {
	...
	for (var i = 0; i < grid.length; i++) {
		...
		for (var j = 0; j < grid[i].length; j++) {
			if (i >= xMin_ && i <= (xMin_+width) && j >= yMin_ && j <= (yMin_+height)) {
...
function createPuzzle() {
	...
	rectArray.push(rect);
	var test, angle, index, x1, x2, x3, x4, y1, y2, y3, y4, rMin, rMax, r, newRect, counter;
...
export class VolcanoFrenzy extends React.Component {
    ...
    render() {
        return (
            <div>
                <canvas id="scrollPreventCanvas" width="1136" height="640" style={{position: "absolute", top: "0", left: "0", zIndex: 0, border: "0px solid black"}} />
                <canvas id="lavaCanvas" width="1136" height="640" style={{position: "absolute", top: "0", left: "0", zIndex: 1, border: "0px solid black"}} />
                <canvas id="cavernCanvas" width="1136" height="640" style={{position: "absolute", top: "0", left: "0", zIndex: 2, border: "0px solid black"}} />
                <canvas id="fallingCanvas" width="1136" height="640" style={{position: "absolute", top: "0", left: "0", zIndex: 3, border: "0px solid black"}} />
                <canvas id="tileCanvas" width="1136" height="640" style={{position: "absolute", top: "0", left: "0", zIndex: 4, border: "0px solid black"}} />
                <canvas id="goldCanvas" width="1136" height="640" style={{position: "absolute", top: "0", left: "0", zIndex: 5, border: "0px solid black"}} />
                <canvas id="cartCanvas" width="1136" height="640" style={{position: "absolute", top: "0", left: "0", zIndex: 6, border: "0px solid black"}} />
                <canvas id="placardCanvas" width="1136" height="640" style={{position: "absolute", top: "0", left: "0", zIndex: 7, border: "0px solid black"}} />
            </div>
        );
    }
}