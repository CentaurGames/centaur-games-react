...
var scrollPreventCanvas, trackCanvas, carCanvas, imageCanvas, screenCanvas, backCanvas;

var trackContext, carContext, imageContext, screenContext, backContext; 
...
var carImage1, carImage2, carImage3, replayImage;
...
function rescaleCanvases() {
	...
	var offsetX = Math.round(scale*150.);
	...
	screenCanvas.width = Math.round(1000.*scale);
	screenCanvas.height = Math.round(700.*scale);
	screenContext.scale(scale,scale);
	backCanvas.width = Math.round(1000.*scale);
	...
	if (gameOver) {
		screenContext.drawImage(replayImage,450,300);
	}
}
...
function drawScore() {
	screenContext.clearRect(0,0,1000,700);
	screenContext.strokeStyle = "black";
	screenContext.fillStyle = "white";
	screenContext.lineWidth = 1;
	screenContext.font = "30px helvetica";
	...
	screenContext.textBaseline = "middle";
	screenContext.strokeText("Current Score: " + score ,950,50);
	screenContext.fillText("Current Score: " + score ,950,50);
	drawBestScore();
}

function detectCollision() {
	if ((theta1+phi) >= theta2 && (theta1-phi) <= theta2 && (R1 === R2 || R1 === R3)) {
	...
	if (theta1 >= 2*Math.PI - 2*phi && (R1 === R2 || R1 === R3)) {
		if (isCarTimerRunning) {
			clearInterval(carTimer);
			isCarTimerRunning = 0;
		}
...
function drawCars() {
	...
	carContext.translate(350,350);
	carContext.rotate(theta2);
	carContext.drawImage(carImage3,-55,-R3-50);
	carContext.restore();
}
...
function createBackTrack() {
	backContext.clearRect(0,0,1000,700);
	var grad = backContext.createLinearGradient(0,0,1000,700);
	grad.addColorStop(0,"rgb(0,140,0)");
	//grad.addColorStop(0,"yellowgreen");
	grad.addColorStop(0.8,"rgb(0,51,0)");
	backContext.fillStyle = grad;
	backContext.fillRect(0,0,1000,700);
	backImage = new Image(1000,700);
...
function createRaceTrack() {
	...
	*/
	trackContext.fillStyle = "rgb(120,120,120)";
	...
	trackContext.beginPath();
	trackContext.moveTo(trackCanvas.width/2,trackCanvas.height/2);
	trackContext.arc(trackCanvas.width/2,trackCanvas.height/2,r3,0,2*Math.PI);
	trackContext.closePath();
	trackContext.fill();
...
function createTrackMarks() {
	...
	trackContext.lineWidth = 3;
	trackContext.beginPath();
	for(var i = 0; i < 2*Math.PI; i += Math.PI/15.) {
		trackContext.moveTo(r3*Math.cos(i)+trackCanvas.width/2,r3*Math.sin(i)+trackCanvas.height/2);
		trackContext.arc(trackCanvas.width/2,trackCanvas.height/2,r3,i,i+Math.PI/35.);		
	}
	trackContext.stroke();
}
...
function createStartRace() {
	...
	trackContext.fillRect(trackCanvas.width/2 - 20,trackCanvas.height/2 - r2,40,220);
	trackContext.strokeStyle = "white";
	trackContext.lineWidth = 3;
	trackContext.strokeRect(trackCanvas.width/2 - 20, trackCanvas.height/2 - r2+1, 40, 219);
...
function createCar3() {
	...
	//back blue under red design
	var grad5 = carContext.createLinearGradient(29,0,58,0); 
	grad5.addColorStop(0,"orange");
	grad5.addColorStop(0.6,"green");  
	carContext.fillStyle = grad5;
	carContext.fillRect(29,7,29,36);
	//red upper body design
	var grad = carContext.createLinearGradient(19,0,98,0); 
	//grad.addColorStop(0.8,"rgb(240,0,0)"); 
	grad.addColorStop(0,"rgb(179,179,0)");
	grad.addColorStop(1,"rgb(255,255,128)");  
	carContext.fillStyle = grad;
	carContext.beginPath();
	carContext.moveTo(12,7);
	carContext.bezierCurveTo(15,30,35,7,58,7);
	...
	//front bender
	var grad1 = carContext.createLinearGradient(87,0,104,0);
	grad1.addColorStop(0,"rgb(255,255,153)");
	grad1.addColorStop(1,"rgb(179,179,0)");  
	carContext.fillStyle = grad1;
	carContext.beginPath();
	carContext.moveTo(87,0);
	carContext.lineTo(102,0);
	...
	carContext.fill();
	carContext.restore();
	carImage3 = new Image(110,100);
	carImage3.src = carCanvas.toDataURL("image/png");
	carImage3.onload = createBackTrack;
...
export class UltimateRocketRacers extends React.Component {
    ...
    render() {
        return (
            <div>
            <canvas id ="scrollPreventCanvas" width="700" height="700" style={{position: "absolute", top: "0", left: "0", zIndex: 0, border: "0px solid black"}} />
            <canvas id ="trackCanvas" width="700" height="700" style={{position: "absolute", top: "0", left: "0", zIndex: 3, border: "0px solid black"}} />
            <canvas id ="backCanvas" width="1000" height="700" style={{position: "absolute", top: "0", left: "0", zIndex: 2, border: "0px solid black"}} />
            <canvas id ="carCanvas" width="110" height="100" style={{position: "absolute", top: "0", left: "0", zIndex: 4, border: "0px solid black"}} />
            <canvas id ="imageCanvas" width="100" height="100" style={{position: "absolute", top: "100", left: "100", zIndex: 1, border: "0px solid black"}} />
            <canvas id ="screenCanvas" width="1000" height="700" style={{position: "absolute", top: "0", left: "0", zIndex: 5, border: "0px solid black"}} />
            </div>
        );
    }
}
