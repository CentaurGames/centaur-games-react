...
function gyroscopeEvent(thetaX,thetaY) {
	var normalizedPhi;
	if (window.outerWidth > window.outerHeight) {
		//landscape mode
		normalizedPhi = thetaX/180;
	} else {
		//portrait mode
		//normalizedPhi = thetaY/90;
		normalizedPhi = thetaX/180;
	}
	playerX = canvasWidth/2*(1+normalizedPhi);
	playerX = (playerX < playerWidth/2 ? playerWidth/2:playerX);
	playerX = (playerX > (canvasWidth - playerWidth/2 - 1) ? (canvasWidth - playerWidth/2 - 1):playerX);
}
...
export class LumpyDumplings2 extends React.Component {
    componentDidMount() {
...
export {LumpyDumplings2 as default};