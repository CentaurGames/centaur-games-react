...
function touchButton(e)
{
	...
	if (x >= 0 && x <= canvasWidth && y >= 0 && y <= canvasHeight)
	{
		...
		if (GAME_PAUSED)
		{
			...
			key = {evnt:e, keyCode:code, preventDefault:function() { this.evnt.preventDefault(); }};
			click_stuff(key);
		}
	} else {
		drawButton();
...
function click_stuff(e)
{
	...
	switch (e.keyCode)
	{
		case 37:
			e.preventDefault();
			if (snakex[0]-1 == snakex[1]) {return;}
			VX = -1;
			VY = 0;
			break;
		case 38:
			e.preventDefault();
			if (snakey[0]-1 == snakey[1]) {return;}
			VX = 0;
			VY = -1;
			break;
		case 39:
...
export class Snake extends React.Component {
    ...
    render() {
        return (
            <div>
            <canvas id="scrollPreventCanvas" width="400" height="400" style={{position: "absolute", left: "0", top:"0", zIndex: 0, border: "0px solid black"}} />
            <canvas id="myCanvas" width="400" height="400" style={{position: "absolute", left: "0", top:"0", zIndex: 2, border: "1px solid black"}} />
            <canvas id="background" width="400" height="400" style={{position: "absolute", left: "0", top: "0", zIndex: 1, border: "1px solid black"}} />
            <canvas id="playButton" width="400" height="400" style={{position: "absolute", left: "0", top: "0", zIndex: 3, border: "1px solid black"}} />
            <canvas id="txtCanvas" width="400" height="400" style={{position: "absolute", left: "0", top: "0", zIndex: 4, border: "1px solid black"}} />
            </div>
        );
    }
}
