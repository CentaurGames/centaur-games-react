...
describe("BlogComponent component", () => {
  ...
  it.each(blogNames.map((blogName, index) => ({ blogName, index })))(
    "renders appropriate information for %s",
    ({ blogName, index }) => {
      ...
      if (previousBlogName) {
        ...
        expect(previousBlog.getAttribute("href")).toEqual(
          getBlogUrl(previousBlogName)
        );
      }

      if (gamesMentioned.length) {
        const gamesMentionedInThisPost = getByText(
          "Games Mentioned in This Post:"
        );
        expect(gamesMentionedInThisPost).not.toBeNull();
      } else {
        const gamesMentionedInThisPost = queryByText(
          "Games Mentioned in This Post:"
        );
