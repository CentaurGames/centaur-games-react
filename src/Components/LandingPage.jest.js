...
describe("LandingPage component", () => {
    ...
    it.each(Object.keys(GAME_META_INFO))("displays %s correctly", (gameName) => {
        ...
        const {iosURL, androidURL, recommendedGames, screenshotImages} = GAME_META_INFO[gameName];

        const playGame = getByText("Play Game");
        expect(playGame.parentElement.getAttribute("href")).toEqual(BASE_URL + "?pageType=game&pageName=" + gameName);
        ...
        const recommendedGameUrls = queryAllByTestId("icon-landing-page").map(icon => icon.getAttribute("href"));
        expect(recommendedGameUrls.length).toEqual(recommendedGames.length);

        for (let i = 0; i < recommendedGames.length; i++) {
            const url = BASE_URL + "?pageType=landing&pageName=" + recommendedGames[i];
