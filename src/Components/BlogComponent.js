...
export class BlogComponent extends React.Component {
    ...
    render() {
        ...
        const nextBlogName = index === blogNameArray.length-1 ? null : blogNameArray[index+1];
        const previousBlogName = index === 0 ? null : blogNameArray[index-1];
        return (
            <div>
                <MenuBar />
                <div className="blog-title">{BLOG_META_INFO[this.props.blogName].title}</div>
                {this.generateBlog()}
                {BLOG_META_INFO[this.props.blogName].gamesMentioned.length > 0 && (
                    <div>
                        ...
                        <div className="recommended-games">
                            {BLOG_META_INFO[this.props.blogName].gamesMentioned.map(gameName => (
                ...
                <div className="sub-paragraph">
                    {nextBlogName && (
                        <a className="next-blog" href={getBlogUrl(nextBlogName)}>Next Blog >></a>
                    )}
                    {previousBlogName && (
                        <a className="previous-blog" href={getBlogUrl(previousBlogName)}>{"<< Previous Blog"}</a> 
                    )}
                </div>
                <SocialMediaIcons />
