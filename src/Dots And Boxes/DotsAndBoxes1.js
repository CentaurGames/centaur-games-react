...
function generateGrid(rowNum) {
  return [...Array(rowNum)].map(() =>
    [...Array(rowNum)].map(() => INITIAL_CELL_STATE)
  );
}
...
export class DotsAndBoxes1 extends React.Component {
  ...
  winningMove() {
    ...
    for (let i = 0; i < grid.length; i++) {
      for (let j = 0; j < grid[i].length; j++) {
        ...
        const newArr = borders.map(border => ({
          side: border,
          color: grid[i][j][border]
        }));
  ...
  isBorderOKToClick(border, i, j) {
    ...
    const borders = ["left", "right", "top", "bottom"];
    let yellowBorders = borders.filter(
      bord => this.state.grid[i][j][bord] === NEUTRAL_BORDER
    ).length;
    if (yellowBorders <= 2) {
      return false;
    }
    switch (border) {
      case "left":
        ...
        if (j - 1 >= 0) {
          yellowBorders = borders.filter(
            bord => this.state.grid[i][j - 1][bord] === NEUTRAL_BORDER
          ).length;
          if (yellowBorders <= 2) {
            return false;
          }
      ...
      case "top":
        ...
        if (i - 1 >= 0) {
          yellowBorders = borders.filter(
            bord => this.state.grid[i - 1][j][bord] === NEUTRAL_BORDER
          ).length;
          if (yellowBorders <= 2) {
            return false;
          }
  ...
  losingMove() {
    ...
    for (let i = 0; i < grid.length; i++) {
      for (let j = 0; j < grid[i].length; j++) {
        ...
        const borders = ["left", "right", "top", "bottom"];
        const clickableBorders = borders.filter(border =>
          this.isBorderOKToClick(border, i, j)
        );
  ...
  cellDidUpdate(grid, cell, i, j, prevState) {
    ...
    if (
      cell.left !== NEUTRAL_BORDER &&
      cell.right !== NEUTRAL_BORDER &&
      cell.top !== NEUTRAL_BORDER &&
      cell.bottom !== NEUTRAL_BORDER
    ) {
      grid[i][j].background = prevState.isPlayer
        ? PLAYER_GRADIENT
        : OPPONENT_GRADIENT;
      return true;
    }
  ...
  playerHover(evt, cell, i, j) {
    ...
    const width = document
      .querySelector(`div[data-id="${dataId}"]`)
  ...
  render() {
    return (
      <div className="dots-and-boxes__background">
        <p className="dots-and-boxes__counter">Select below number of rows</p>
        <CounterInput
          count={5}
          min={3}
          max={15}
        ...
        <div className="dots-and-boxes__game">
          {this.state.grid.map((row, i) => (
            <div className="dots-and-boxes__row" key={i}>
              {row.map((cell, j) => (
                <div
                  className="dots-and-boxes__aspect-ratio"
                ...
                >
                  <div
                    ...
                    onClick={evt => this.clickSquare(evt, cell, i, j)}
                    onMouseMove={evt => this.playerHover(evt, cell, i, j)}
                  />
                </div>
              ))}
            </div>
