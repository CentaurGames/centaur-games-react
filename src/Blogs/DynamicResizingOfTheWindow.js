    ...
    return (
      <div className="blog-text">
        <p className="sub-paragraph">
          ...
          automatically fit the screen whenever the window is resized. There is
          a school of thought in web development that the user should have
          ultimate control over every aspect of a webpage - including its size -
          but that school of thought largely flies out the window when
          discussing a game, for the very practical reason that it confuses game
          controls with web page controls. For example, a swipe in a game is
          most likely intended to move the character, as opposed to causing the
          web page to scroll past the game. (This is especially true if your
          game is the only element on the page, as is the case with our games.)
          Likewise, some games require two fingers moving in unison to operate
          the controls: one which tells the character what direction to move,
          and one which tells the character what direction to shoot. Clearly
          these games do not want the web page interpreting "two fingers
          dragging" as zooming/shrinking the window.
        </p>
        <p className="sub-paragraph">
          ...
          our games remain constantly centered with respect to the screen, and
          are always scaled such that they most closely fit the screen at its
          current orientation. (The user can of course change the size by
          rotating the screen 90 degrees, if on a mobile device, but any game
          using gyroscope properties should disable this feature as well. We
          won't cover that in this post as none of our games use the gyroscope
          events.)
        </p>
        <p className="sub-paragraph">
        ...
        <p className="sub-paragraph">
          ...
          there's a catch: first you need to disable the scrollbar. Once again,
          we are assuming that the user has no use for it - after all, the game
          will appear exactly centered no matter how much they scroll it, so it
          is a useless feature to have at this point. You can enable the
          scrollbar, but your window width and height will be slightly incorrect
          on some devices and you will wind up with a scrollbar covering part of
          your game. Trust us: your users will thank you to get rid of it.
        </p>
        <p className="sub-paragraph">
        ...
        <p className="sub-paragraph">
          ...
          <span style={{ color: "#00ffff" }}>window.scrollTo(0,0)</span> to
          scroll to the top of the page, and set each canvas's top left corner
          to (0,0). Each time you center the canvas, it gets pushed down the
          page a little bit. If you do not recalibrate its top-left corner each
          time you center it, the canvas will eventually get pushed beyond the
          edge of the page's limits and you won't be able to see it.
        </p>
        <p className="sub-paragraph">
          Once you have calibrated the top-left corner of the canvas, you need
          to find the position that this top-left corner actually occupies on
          your web page. Even though you have set it to (0,0), your web page may
          have an internal lower limit that gets in the way of this. For
          example, WordPress does not allow us to put our canvases at the very
          top because that space is occupied by our menu. Thus even when we have
          specified (0,0) as the top left corner, a closer examination will
          reveal that our canvas is located somewhere else. To find its actual
          location, we need to use 
          <span style={{ color: "#00ffff" }}>getBoundingClientRect</span> and
          ask for the top left corner. The values this returns will represent
          the shift that we need to correct for later.
        </p>
        <p className="sub-paragraph">
          Upon centering our canvas, there should be just as much space above
          its top side as below its bottom side. This means that the top is
          located{" "}
          <span style={{ color: "#00ffff" }}>
            (window.innerHeight-canvas.height)/2
          </span>{" "}
          pixels away from the top of the page. Likewise, the left side is
          located{" "}
          <span style={{ color: "#00ffff" }}>
